...
| path split | router-side `latency` minus `moderation_latency` from the `/generation` ledger, at two pinned prompt sizes, bucketed | it measures the router→PROVIDER leg, not your own distance to the edge: the network floor is the upstream's distance class, the prefill slope its accelerator |
