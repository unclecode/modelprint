...
async function runModel(model) {
  ...
  for (const { meta, probe } of probes) {
    try { out[meta.id] = (await probe({ chat: p => chat(model, p), http: httpGet, model })).value; }
...
for (const k of ids) console.log(`  ${k.padEnd(20)} ${String(ox[k]).slice(0, 90)}`);
...
for (const { m, hits, tok } of rank)
  console.log(`  ${String(hits).padStart(2)}/${ids.length}  tokenizer ${tok}/4  ${m}`);
